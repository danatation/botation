...
class CaptchaGiver(commands.Cog):
	...
	async def on_message(self, message: discord.Message):
		if random.randint(0, 100) == 0 and message.author != self.bot.user:
			view = CaptchaButton()
			await message.reply(
				'We have detected abnormal activity from your device. \
				You\'ll need to verify if you\'re a human by solving an anonymous Captcha.',
				view=view)
...
class CaptchaModal(ui.Modal, title='Captcha'):
	...
	math_challenge = ui.TextInput(
		label=f'Solve {random.randint(0, 100)} {random.choice(['+', '-', '/', '*', '^'])} {random.randint(0, 100)}')
	...
	async def on_submit(self, interaction: discord.Interaction) -> None:
		...
		await interaction.response.send_message(
			f'User {interaction.user.mention} has answered the Capthca. Here are their answers!', embed=embed)
		if isinstance(interaction.channel, discord.TextChannel):
			await interaction.channel.send(
				'You have not passed the Captcha. A human worker will evaluate your account and determine if you\'re a real person.')
